...
def gen_n_fact(tdf, ticker, n=3):
    s = tdf[ticker]
    l = [s.quantile(q) for q in np.linspace(0, 1, n + 1)]
    limits = [(l[i:i + 2]) for i in range(n)]
    df = pd.DataFrame()
    df['factor'] = np.zeros(tdf.shape[0])
    df.index = s.index

    for i, l in enumerate(limits):
        df['factor'][(s > l[0]) & (s <= l[1])] = i

    cat_features = to_categorical(df['factor'], num_classes=n)
    for i in range(n):
        df[f'f{i}'] = cat_features[:, i]
    factor = df['factor']
    df.drop(columns=['factor'],inplace=True)
    return pd.concat([tdf, df], axis=1), df,factor


def gen_fact(tdf, ticker, delta=.1):
    s = tdf[ticker]
    ll, hl = s.quantile(.5 - delta), s.quantile(.5 + delta)
    r0 = s < ll
    r1 = s > hl
    df = pd.DataFrame()
    df['factor'] = np.zeros(tdf.shape[0])
    ...
    cat_features = to_categorical(df['factor'], num_classes=3)
    for i in range(3):
        df[f'f{i}'] = cat_features[:, i]
    factor = df['factor']
    df.drop(columns=['factor'], inplace=True)
...
def series_visualise(y,lags=100,title='Series'):
    figsize=(12,8)
    _,axs = plt.subplots(nrows=3,ncols=2,figsize=figsize)

    y.plot(ax=axs[0,0])
    axs[0,0].set_title(title)

    y.plot(ax = axs[0,1],kind='hist',bins=50)
    axs[0,1].set_title('Histogram')

    sm.graphics.tsa.plot_acf(y,lags=lags,ax=axs[1,0])
    axs[1,0].set_title('AC')

    sm.graphics.tsa.plot_pacf(y,lags=lags,ax=axs[1,1])
    axs[1,1].set_title('PAC')

    [ax.set_xlim(0) for ax in [axs[1,0],axs[1,1]]]
    
    stats.probplot(y, dist="norm", plot=axs[2,0])
